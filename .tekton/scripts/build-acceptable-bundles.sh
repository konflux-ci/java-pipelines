...
pr_number=$(gh search prs --repo konflux-ci/java-pipelines --merged "${REVISION}" --json number --jq '.[].number')

# changed files in a PR
mapfile -t changed_files < <(gh pr view "https://github.com/konflux-ci/java-pipelines/pull/${pr_number}" --json files --jq '.files.[].path')
